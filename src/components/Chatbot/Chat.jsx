...
"${query}"`
  ...
  useEffect(() => {
    const checkLandscape = () => {
      // Check if width is mobile-like (< 900) AND height is short (< 500)
        ...
        >
          <div className="p-4">
            <button
              ...
              className={`flex items-center gap-2.5 bg-[#006D5B] hover:bg-[#128c7e] text-white border-none rounded-full w-full shadow-md transition-colors duration-200 font-bold ${isLandscapeMobile ? 'py-2 px-3 text-xs' : 'py-3 px-4 text-sm'}`}
            >
            ...
            {chatHistoryList.map(chat => {
              ...
              return (
                <div key={chat.id}
                  ...
                  className={`group flex items-center justify-between gap-2.5 mb-1 rounded-full cursor-pointer transition-all duration-200 
                    ${isLandscapeMobile ? 'px-2 py-1.5 text-xs' : 'px-3 py-2.5 text-[13px]'}
                    ${isActive
        ...
        {mobileMenuOpen && (
          <div className="absolute inset-0 z-[200] bg-black/60 backdrop-blur-sm" onClick={() => setMobileMenuOpen(false)}>
            <div className={`w-[85%] max-w-[300px] h-full bg-[#006D5B] text-white shadow-2xl flex flex-col ${isLandscapeMobile ? 'p-3' : 'p-5'}`} onClick={(e) => e.stopPropagation()}>
              <button
                onClick={startNewChat}
                className={`w-full border-none rounded-full font-medium text-white bg-[#006D5B] hover:bg-[#128c7e] ${isLandscapeMobile ? 'mb-3 p-2.5 text-sm' : 'mb-5 p-4 text-base'}`}
              >
                + New Chat
              </button>
              <div className="mt-2.5 flex-1 overflow-y-auto custom-scrollbar-sidebar">
                ...
                {chatHistoryList.map(chat => (
                  <div key={chat.id} onClick={() => loadOldChat(chat.id)} 
                    className={`border-b border-[#075e54] cursor-pointer flex items-center justify-between gap-2.5 
                    ${isLandscapeMobile ? 'p-2 text-xs' : 'p-3.5 text-[15px]'}
                    ${sessionId === chat.id ? "text-white font-semibold bg-[#075e54]" : "text-[#e9edef]"}`}
                  >
                    <div className="flex items-center gap-2.5 overflow-hidden">
                      <span className="truncate">{chat.title}</span>
                    </div>
            ...
            <div className={`w-full max-w-3xl mx-auto flex flex-col ${isLandscapeMobile ? 'py-2 gap-3' : 'py-5 pb-10 gap-6'}`}>
              ...
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex gap-3 w-full ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`
                      flex-initial max-w-[85%] min-w-0 ${isLandscapeMobile ? 'p-2 px-3' : 'p-3 px-4.5'} rounded-t-2xl 
                      ${msg.role === "user"
      ...
      {thinkingTexts.map((text, idx) => {
        ...
        return (
          <motion.div
            ...
            className={`flex items-start gap-4 relative ${isLandscapeMobile ? 'min-h-[25px]' : 'min-h-[40px]'}`}
          >
            ...
            <motion.div 
              ...
              className={isLandscapeMobile ? 'pb-2' : 'pb-4'}
            >
              <span className={`${isLandscapeMobile ? 'text-xs' : 'text-sm'} text-[#54656f] italic font-medium tracking-tight`}>
                {text}
              </span>
            </motion.div>
          </motion.div>
            ...
            <div className="w-full max-w-3xl relative">
              ...
              <div className={`mb-3 flex flex-wrap ${isLandscapeMobile ? 'gap-1.5 mb-1.5' : 'gap-2.5 mb-3'}`}>
                <select
                  ...
                  className={`rounded-full border border-[#e9edef] bg-white text-[#111b21] outline-none cursor-pointer shadow-sm hover:bg-gray-50 transition-colors ${isLandscapeMobile ? 'py-1 px-2 text-xs min-h-[30px]' : 'py-2.5 px-4 text-sm min-h-[40px]'}`}
                >
                ...
                <select
                  ...
                  className={`rounded-full border border-[#e9edef] bg-white text-[#111b21] outline-none cursor-pointer shadow-sm hover:bg-gray-50 transition-colors ${isLandscapeMobile ? 'py-1 px-2 text-xs min-h-[30px]' : 'py-2.5 px-3 text-sm min-h-[40px]'}`}
                >
                ...
                <div className="absolute right-1.5 top-1/2 -translate-y-1/2 z-20 flex gap-1.5">
                  {browserSupportsSpeechRecognition && (
                    <button
                      ...
                      className={`
                          rounded-full flex items-center justify-center transition-all duration-200 border-b-2 border-black/10
                    ...
                    >
                      {listening ? (
                        <svg width={isLandscapeMobile ? "14" : "20"} height={isLandscapeMobile ? "14" : "20"} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="6" y="6" width="12" height="12" /></svg>
                      ) : (
                        <svg width={isLandscapeMobile ? "14" : "20"} height={isLandscapeMobile ? "14" : "20"} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z" /><path d="M19 10v2a7 7 0 0 1-14 0v-2" /><line x1="12" y1="19" x2="12" y2="23" /><line x1="8" y1="23" x2="16" y2="23" /></svg>
                      )}
                    </button>
                  )}
